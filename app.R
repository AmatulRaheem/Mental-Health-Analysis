...
survey_final <- survey_final %>%
  filter(Gender != "a little about you",
         Gender != "p")
survey_final$Gender <- sapply(as.vector(survey_final$Gender), function(x) if(x %in% male_str) "Male" else x )
survey_final$Gender <- sapply(as.vector(survey_final$Gender), function(x) if(x %in% female_str) "Female" else x )
survey_final$Gender <- sapply(as.vector(survey_final$Gender), function(x) if(x %in% trans_str) "Trans" else x )
View(survey_final)
summary(survey_final)
str(survey_final)


#CHECK FINAL DATASET GENDER COLUMN
unique(survey_final$Gender)

#CHECK MISSING VALUES
sapply(survey_final, function(x) sum(is.na (x)))
sum(is.na(survey_final$Country=="United States")) #EVERYONE FROM USA HAS MENTIONED THEIR STATE LOCATION

#CHECKING TO REMOVE REST OF THE NA VALUES, WE KEEP STATE NA VALUES AS ONLY USA HAS STATE DISTINCTION
unique(survey_final$self_employed) #DROP NA
unique(survey_final$work_interfere) #DROP THIS COLUMN

View(survey_final)
summary(survey_final)


#DROP NA FROM SELF_EMPLOYED AND DELETE COLUMN WORK_INTERFERE
library(tidyr)
library(tidyverse)

#REMOVE MISSING VALUES FROM SELF_EMPLOYED
survey_final[!complete.cases(survey_final$self_employed),]
survey_final = survey_final[complete.cases(survey_final$self_employed),]
unique(survey_final$self_employed) #CHECK

#DELETE COLUMN WORK_INTERFERE
library(dplyr)
survey_final = select(survey_final, -6,)
survey_final
View(survey_final)

#FINAL DATASET EXPORTING
write.csv(survey_final,"survey_final.csv")

data <- read.csv("survey_final.csv")
...
mapStates = map("world", fill = TRUE, plot = FALSE)
mapStates
geo_location <- read.csv("countries.csv")
geo_location
